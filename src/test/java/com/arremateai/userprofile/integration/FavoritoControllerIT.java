...
@DisplayName("FavoritoController - Integração")
class FavoritoControllerIT extends AbstractIntegrationTest {
    ...
    private UUID criarUsuario() throws Exception {
        ...
        UsuarioSyncRequest sync = new UsuarioSyncRequest(
                id, "Fav User", "fav-" + id + "@teste.com", null, null, "COMPRADOR"
        );
    ...
    @Test
    @DisplayName("GET /api/favoritos sem gateway auth retorna 401")
    ...
    @Test
    @DisplayName("GET /api/favoritos/count retorna 0 para usuário novo")
    void deveRetornarZeroFavoritos() throws Exception {
        UUID id = criarUsuario();
        mockMvc.perform(get("/api/favoritos/count")
                        .header("X-Gateway-Auth", "test-gateway-secret")
    ...
    @Test
    @DisplayName("GET /api/favoritos/{id}/status retorna falso para imóvel inexistente")
    void deveVerificarFavoritoInexistente() throws Exception {
        UUID id = criarUsuario();
        UUID imovel = UUID.randomUUID();
        mockMvc.perform(get("/api/favoritos/" + imovel + "/status")
                        .header("X-Gateway-Auth", "test-gateway-secret")
                        .header("X-User-Id", id.toString()))
                .andExpect(status().isOk())
